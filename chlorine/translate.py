...
class Translate(object):
    ...
    def translate(self) -> str:
        ...
        if re.search('.*?(是什么|是什么意思)$', string) is not None:
                return string

        if string.endswith('是什么东西'):
                return string[:-5] + '是什么'

        if (res := re.search('.*?是啥玩意(儿?)$', string)) is not None:
            index = -4
            if res.group(1) == '儿':
                index = -5
            return string[:-index]

        if (res := re.search('.*?是什么玩意(儿?)$', string)) is not None:
                index = -5
                if res.group(1) == '儿':
                    index = -6
                return string[:-index]
